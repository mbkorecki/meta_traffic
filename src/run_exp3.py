...
#         os.system("python traffic_sim.py --sim_config \"" + path + d + ".config\"" + " --num_sim_steps 3600 --num_episodes 1 --lr 0.0005 --agents_type analytical --path \"" + results_path+"/"+d + "\"")
...
ax.errorbar(np.asarray(ds), [np.mean(y) for y in data], yerr=err, marker='+', linestyle='solid', transform=trans1, label='Analytic+', capsize=3, ms=2, c='xkcd:aqua')
ax.errorbar(np.asarray(ds), generalight_means, yerr=generalight_err, marker='x', linestyle='dashed', transform=trans2, label='Generalight', capsize=3, ms=2, c ='xkcd:bubblegum')
ax.errorbar(np.asarray(ds), colight_means, yerr=colight_err, marker='o', linestyle='dotted', transform=trans4, label='Colight', capsize=3, ms=2, c='xkcd:steel')
ax.errorbar(np.asarray(ds), metalight_means, yerr=metalight_err, marker='D', linestyle='dashdot', transform=trans3, label='Metalight', capsize=3, ms=2, c='xkcd:pale purple')
...
plt.tight_layout()
plt.savefig("atlanta.pdf", format="pdf", bbox_inches="tight")
