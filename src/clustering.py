...
class Mfd_Clustering():
    ...
    def __init__(self, cluster_models):
        ...
        M = []
        for i in range(4):
            cluster_models.add_model(i)
            M.append(MicroCluster(centroid=0,ID=i))
        self.M.append(M)
        
    def process(self, flow_change, density_change):
        if flow_change >= 0 and density_change >= 0:
            self.M[-1][0].number_points += 1
            return 0
        elif flow_change < 0 and density_change < 0:
            self.M[-1][1].number_points += 1
            return 1
        elif flow_change >= 0 and density_change < 0:
            self.M[-1][2].number_points += 1
            return 2
        elif flow_change < 0 and density_change >= 0:
